...
# (skip if only plotting) generate additional site data to be used in analyses
...
site_data_A = readRDS(here('output_data', 'lotic_site_info_filtered.rds')) %>%
    as_tibble() %>%
...
map = tm_shape(basemap) + tm_rgb() +
    tm_shape(sites_sp) + tm_symbols(shape = 21, col = spcolor, size = 0.3,
                                    border.lwd = 1, border.col = 'black') +
    tm_shape(sites_fnet) + tm_symbols(shape = 21, col = fnetcolor, size = 0.3,
                                      border.lwd = 1, border.col = 'black') +
    ...
    tm_add_legend(type = 'symbol', labels = 'StreamPULSE', col = spcolor,
                  shape = 21, border.col = 'black') +
