...
* **Part III. Plotting and export of stats dataset:** Datasets from parts I and II are minimally subsetted and recast for plotting convenience. Figures 1-6 are generated. NHDPlusV2 supplementary data (slope, area, stream order) are acquired for each site. An analysis-ready dataset is compiled.

* **Part IV. Analysis:** (ADD TEXT)
...
knitr::opts_knit$set(root.dir = '/home/mike/git/streampulse/metab_synthesis_2')
```

```{r setup2, include = FALSE}
if(! file.exists(".here")){here::set_here('.')}
...
coverage_fnet = readRDS('output_data/fluxnet_site_info_filtered.rds') %>%
    as_tibble() %>%
    select(sitecode = Site_ID,
...
if(got_reach_proportions){
    site_data_2$reach_proportion = readRDS(here('output_data', 'spatial/reach_proportion_reference.rds'))
} else {
    ...
    saveRDS(site_data_2$reach_proportion, here('output_data', 'spatial/reach_proportion_reference.rds'))
}
